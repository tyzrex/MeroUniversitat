...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="group/copy inline-flex items-center gap-1.5 rounded-lg bg-muted px-2.5 py-1.5 font-mono text-xs font-semibold text-foreground transition-all hover:bg-muted/80"
    ...
    >
      ...
      {copied ? (
        ...
        <Copy
          className="size-3 text-muted-foreground transition-colors group-hover/copy:text-foreground"
          strokeWidth={1.8}
        />
      )}
    </button>
      ...
      <div className="border-b border-border px-6 py-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              className="border-input bg-background h-10 rounded-xl border px-3 text-sm font-semibold text-foreground shadow-xs focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none"
              value={role}
              onChange={(e) =>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead className="border-b border-border bg-muted/60">
            <tr>
              <th className="px-6 py-3 font-semibold text-foreground">Team</th>
              <th className="px-6 py-3 font-semibold text-foreground">Role</th>
                  ...
                  >
                        ...
                        <div className="flex -space-x-1.5">
                          {r.members.slice(0, 4).map((m, i) =>
                            m.image ? (
                              ...
                              <span
                                key={`${r.id}-${i}`}
                                className="flex size-6 items-center justify-center rounded-full border-2 border-background bg-muted text-[9px] font-bold text-muted-foreground"
                                title={m.name}
                              >
                          ...
                          {r.memberCount > 4 ? (
                            <span className="flex size-6 items-center justify-center rounded-full border-2 border-background bg-muted/80 text-[9px] font-bold text-muted-foreground">
                              +{r.memberCount - 4}
                            </span>
                          ) : null}
                        </div>
                    ...
                    <td className="px-6 py-4">
                      <Link
                        ...
                        className={cn(
                          "inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs font-semibold text-primary transition-all hover:bg-primary/10",
                        )}
                      >
                        Manage
