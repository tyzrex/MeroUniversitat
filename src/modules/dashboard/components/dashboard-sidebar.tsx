...
export function DashboardSidebar({
	...
	const showStaffNav = user?.role === 'ADMIN' || user?.role === 'MODERATOR';
	...
	return (
		...
		>
			<SidebarHeader className="gap-3 px-3 pt-4 pb-2">
				<Link
					className="ring-sidebar-ring flex items-center gap-1 rounded-md px-1 py-1 outline-none focus-visible:ring-2"
					href="/dashboard"
				>
					<Image
						alt="MeroUniversität"
									...
									<SidebarMenuItem key={item.href}>
										...
										>
											...
											<span className="flex min-w-0 flex-1 items-center gap-2">
												...
												{item.badge ? (
													<span className="rounded-full bg-blue-400/25 px-1.5 py-0.5 text-[10px] font-semibold tracking-wide text-blue-100 uppercase group-data-[collapsible=icon]:hidden">
														{item.badge}
													</span>
												) : null}
											</span>
