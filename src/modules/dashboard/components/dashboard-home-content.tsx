	...
	return (
		<div className="flex flex-col gap-8">
			<DashboardPageIntro
				...
				description={
					<>
						Deadlines, Kanban, teams, and community outcomes — scoped to you and
						the teams you join.
					</>
				}
			>
				...
				<section className="rounded-3xl border border-amber-200/90 bg-gradient-to-br from-amber-50 via-orange-50/80 to-rose-50/60 p-6 ring-1 ring-amber-100/80 md:p-7">
					<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
						<div className="flex items-start gap-4">
							<div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-white text-amber-700 ring-1 ring-amber-100">
								<UserCircle2 className="size-7" strokeWidth={1.8} />
							</div>
							<div>
								<p className="text-xs font-bold tracking-[0.18em] text-amber-800/90 uppercase">
									Profile
								</p>
								...
								<p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-700">
									Add GPA, degree background, or target intake so
									recommendations and community submissions stay accurate — it
			...
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
				...
				<GradientMetricCard
					...
					hint={stats.teamCount === 0 ? 'Join or create' : 'Active memberships'}
				/>
				<GradientMetricCard
				...
				<section className="grid gap-6 lg:grid-cols-2">
								...
								{stats.statusBreakdown.map(({ status, count }, i) => (
									<div key={status}>
										<div className="mb-1 flex justify-between text-sm">
											...
											<span className="font-bold text-[#0d2145] tabular-nums">
												{count}
											</span>
										</div>
					...
					{stats.upcomingDeadlines.length > 0 ? (
								...
								{stats.upcomingDeadlines.map((row, i) => {
									...
									return (
										<li
											...
											className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/70 bg-white/90 px-4 py-3 ring-1 ring-slate-900/[0.04]"
										>
											...
											<div className="text-right">
												...
												<p className="text-sm font-bold text-[#0d2145] tabular-nums">
						...
						<div className="flex flex-col justify-center rounded-3xl border border-dashed border-slate-200 bg-sky-50/40 p-8 text-center">
							<CalendarClock
								className="mx-auto size-10 text-sky-200"
								strokeWidth={1.5}
							/>
							<p className="mt-4 font-semibold text-[#0d2145]">
							...
							<p className="text-muted-foreground mt-2 text-sm">
								Add deadline dates on your application rows to see a countdown
								here.
							</p>
						</div>
					)}
			...
			<SimilarPeersPanel userId={userId} />
...
function getQuickActions(preference: string | null, teamCount: number) {
	const actions: {
		title: string;
		description: string;
		href: string;
